...
import {
  ...
  genFromWei, fmtUsd, toast, pollWhileVisible, refreshVaultChip, vaultState, openVaultModal,
} from './shared.js';
...
function renderSummary() {
  ...
  // Nothing is ever "to collect" now — winnings are credited at settlement — so the
  // useful figure is what is sitting in the play balance ready to stake again.
  const atRisk = bets
    .filter((b) => b.state === "PENDING" || b.state === "LIVE")
    .reduce((s, b) => s + Number(BigInt(b.amount)) / 1e18, 0);
  ...
  $("summary").innerHTML = [
    ...
    stat("Play balance", `${(Number(vaultState.balance) / 1e18).toFixed(3)} GEN`),
    stat("On open rounds", `${atRisk.toFixed(3)} GEN`, atRisk > 0 ? "var(--warn)" : undefined),
  ].join("");
}
...
(async function init() {
  ...
  await refresh();
  onWalletChange(async () => { await refreshVaultChip(); await refresh(); });
